	...
	function legend(svg) {
		...
		if (orient === "vertical") {
			...
			cellTrans = function (d, i) {
				...
				cell.each(function (cd, ci) {
					if (ci < i) {
						totalCellWidth += this.getBBox().width;
					}
				});
				//当前所有的shape均相同, 故可以直接减去第一个元素的x偏移即可
				return "translate(" + (totalCellWidth + i* shapePadding - shapeSize[0].x)   + ",0)";
